...
In other words, a vertex's transform can be defined as:\[\sum_b{I_bT_b}\] where $b$ is a bone, $I_b$ is the influence of $b$ on the vertex and $T_b$ is the transform matrix of the bone.
...
When rendering the mesh this also requires the vertex data to be re-sent to the graphics card if the skinning calculations were done on the CPU.
...
Hardware skinning is usually implemented as a vertex shader, which is a piece of code executed on the GPU for every vertex in a mesh.
The OpenGL Shading Language (GLSL) will be the shading language used in this work.
...
The NVIDIA example has a maximum of four bones influencing each vertex, which is why weights and indexes are stored in vec4 data types.
